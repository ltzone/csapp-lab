...
mem_t *init_reg(int len)
{
    mem_t * r = init_mem(REG_SIZE);
    // set_reg_val(r, REG_RSP, len-8); 
    return r;
}
...
bool_t should_read_reg(itype_t icode){
    bool_t should_read = TRUE;
    switch (icode)
    {
    case I_HALT:
    case I_NOP:
...
bool_t should_read_imm(itype_t icode){
    bool_t should_read = FALSE;
    switch (icode)
    {
    case I_IRMOVQ:
    case I_RMMOVQ:
...
void exec_rrmovq(y64sim_t *sim, cond_t ifun, regid_t regA, regid_t regB){
...
bool_t exec_irmovq(y64sim_t *sim, long_t val, regid_t regA, regid_t regB){
    if (regA != REG_NONE)
        return FALSE;
    set_reg_val(sim->r, regB, val);
    return TRUE;
...
bool_t exec_rmmovq(y64sim_t *sim, regid_t regA, regid_t regB, long_t addr){
    long_t val = get_reg_val(sim->r, regA);
    addr += get_reg_val(sim->r, regB);
    return set_long_val(sim->m, addr, val);
}

bool_t exec_mrmovq(y64sim_t *sim, regid_t regA, regid_t regB, long_t addr){
    addr += get_reg_val(sim->r, regB);
    long_t val = 0;
...
void exec_alu(y64sim_t *sim, alu_t ifun, regid_t regA, regid_t regB){
...
bool_t exec_popq(y64sim_t *sim, regid_t regB){
    long_t pop_val;
    long_t rsp_val = get_reg_val(sim->r, REG_RSP);
    if (!get_long_val(sim->m, rsp_val, &pop_val)){
        err_print("PC = 0x%lx, Invalid stack address 0x%lx", sim->pc, rsp_val);
        return FALSE;
    }
    set_reg_val(sim->r, regB, pop_val);
    if (regB != REG_RSP){
        set_reg_val(sim->r, REG_RSP, rsp_val + 8);
    }
    return TRUE;
...
bool_t exec_call(y64sim_t *sim, long_t addr){
    long_t rsp_val = get_reg_val(sim->r, REG_RSP);
    long_t ret_add = sim->pc + 9; 
    set_reg_val(sim->r, REG_RSP, rsp_val - 8);
    if (!set_long_val(sim->m, rsp_val - 8, ret_add)){
        err_print("PC = 0x%lx, Invalid stack address 0x%lx", sim->pc, rsp_val - 8);
        return FALSE;
    }
    sim->pc = addr;
...
stat_t nexti(y64sim_t *sim)
{
    ...
    byte_t codereg = 0;
    regid_t regidA = REG_ERR, regidB = REG_ERR;
    long_t imm_val = 0;
    ...
    if (should_read_reg(icode)){
        if (!get_byte_val(sim->m, next_pc, &codereg)) {
            err_print("PC = 0x%lx, Invalid instruction address", sim->pc);
            return STAT_ADR;
        }
        regidA = GET_REGA(codereg);
        regidB = GET_REGB(codereg);
        next_pc++;
    }
    

    /* get immediate if needed (8 bytes) */
    if (should_read_imm(icode)){
        if (!get_long_val(sim->m, next_pc, &imm_val)) {
            err_print("PC = 0x%lx, Invalid instruction address", sim->pc);
            return STAT_ADR;
        }
        next_pc+=8;
    ...
    switch (icode) {
      ...
      case I_RRMOVQ:  /* 2:x regA:regB */
        exec_rrmovq(sim, (cond_t) ifun, regidA, regidB);
        sim->pc = next_pc;
        break;
      case I_IRMOVQ: /* 3:0 F:regB imm */
        if (!exec_irmovq(sim, imm_val, regidA, regidB))
            return STAT_INS;
        sim->pc = next_pc;
        break;
      case I_RMMOVQ: /* 4:0 regA:regB imm */
        if (!exec_rmmovq(sim, regidA, regidB, imm_val))
            return STAT_ADR;
        sim->pc = next_pc;
        break;
      case I_MRMOVQ: /* 5:0 regB:regA imm */
        if (!exec_mrmovq(sim, regidA, regidB, imm_val))
            return STAT_ADR;
        sim->pc = next_pc;
        break;
      case I_ALU: /* 6:x regA:regB */
        exec_alu(sim, ifun, regidA, regidB);
        sim->pc = next_pc;
        break;
      case I_JMP: /* 7:x imm */
      ...
      case I_PUSHQ: /* A:0 regA:F */
        if(!exec_pushq(sim, regidA))
            return STAT_ADR;
        sim->pc = next_pc;
        break;
      case I_POPQ: /* B:0 regA:F */
        if(!exec_popq(sim, regidA))
            return STAT_ADR;
        sim->pc = next_pc;
    	break;
      default:
